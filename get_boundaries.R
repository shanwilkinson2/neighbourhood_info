...
# lsoa-neighbourhood link new 6 area model
# add value for Bolton to keep whole borough value????
  lsoa_neighbourhood <- readxl::read_xlsx("6 neighbourhoods final option.xlsx") %>%
    janitor::clean_names()
...
# add in neighbourhoood name to lsoa boundaries
  lsoas_bolton2 <- left_join(lsoas_bolton %>%
                             ...
                             lsoa_neighbourhood %>%
                               select(neighbourhood_name = x6_areas_name, neighbourhood_num = x6_areas_number, lsoa_name),
                             by = c("lsoa11cd" = "lsoa_name"))

# get just outer neighbourhood boundary
  neighbourhood_boundaries <- lsoas_bolton2 %>%
    select(neighbourhood_name, neighbourhood_num) %>%
    group_by(neighbourhood_name, neighbourhood_num) %>%
    st_make_valid() %>% # creates an error saying edge has duplicate vertex without this
